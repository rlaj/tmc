define('utils/lb/lb-sticky-nav',['require','backbone','jquery','utils/common','utils/lb/lb-window-size','utils/pubsub'],function(require) {
  ...
  var StickyNav = Backbone.View.extend({
    ...
    render: function() {
      ...
      this.resetTrigger();
      if(LBWin.size() !== LBWin.sizes.smskeuomorphic && LBWin.size() !== LBWin.sizes.lgskeuomorphic) {
        this.stickied = true;
        this.active = true;
        this.initSticky();
      }
    ...
    resizeListener: function() {
      this.resetTrigger();
      if(this.active && LBWin.size() === LBWin.sizes.smskeuomorphic && LBWin.size() === LBWin.sizes.lgskeuomorphic) {
        this.active = false;
        this.stickied = false;
      ...
      } else if(!this.active && LBWin.size() !== LBWin.sizes.smskeuomorphic && LBWin.size() !== LBWin.sizes.lgskeuomorphic) {
        this.active = true;
        this.stickied = true;
        this.initSticky();
      }
